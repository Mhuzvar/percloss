...
def lin2bark_mat(fs=44100, nfft=None):
    if not nfft:
        nfft=int(np.floor(2048*(fs/48000)))
    B_bands = ['0 80 91.708 103.445 23.445',
               '1 103.445 115.216 127.023 23.577',
               '2 127.023 138.87 150.762 23.739',
               '3 150.762 162.702 174.694 23.932',
    ...
    fl = np.linspace(0,fs,nfft, endpoint=False, retstep=True)
    fbins = (nfft//2)+1
    bmat = np.zeros((fbins,109))

    imin = 0
    imax = fbins
    for i in range(len(B_bands)):
        for j in range(len(fl[0])):
            if (fl[0][j]-0.5*fl[1] >= B_bands[i][0]) & (fl[0][j]+0.5*fl[1] < B_bands[i][1]):
                bmat[j][i]+=1
            elif (fl[0][j]-0.5*fl[1] < B_bands[i][0]) & (fl[0][j]+0.5*fl[1] > B_bands[i][1]):
                bmat[j][i]+=(B_bands[i][1]-B_bands[i][0])/fl[1]
            elif (fl[0][j]-0.5*fl[1] < B_bands[i][0]) & (fl[0][j]+0.5*fl[1] > B_bands[i][0]):
                bmat[j][i]+= ((fl[0][j]+0.5*fl[1])-B_bands[i][0])/fl[1]
            elif (fl[0][j]-0.5*fl[1] < B_bands[i][1]) & (fl[0][j]+0.5*fl[1] > B_bands[i][1]):
                bmat[j][i]+= (B_bands[i][1]-(fl[0][j]-0.5*fl[1]))/fl[1]
    ...
    for i in range(fbins):
        ...
        if sum(bmat[-1-i]>0):
            break
    return torch.from_numpy(bmat[imin:imax]).type(torch.float), imin, imax
...
def f_c():
    ...
    f_c = torch.zeros(109, dtype=torch.double)
    for i, line in enumerate(B_bands):
